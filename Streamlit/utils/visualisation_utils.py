...
def plot_confusion_matrices(_model, X, y):
    ...
    disp_train = ConfusionMatrixDisplay(confusion_matrix=cm_train, display_labels=["Term", "Preterm"])
    disp_train.plot(ax=axes[0], cmap="Blues", colorbar=False)
    axes[0].set_title("Train Confusion Matrix")

    disp_test = ConfusionMatrixDisplay(confusion_matrix=cm_test, display_labels=["Term", "Preterm"])
    disp_test.plot(ax=axes[1], cmap="Blues", colorbar=False)
    axes[1].set_title("Test Confusion Matrix")

    disp_all = ConfusionMatrixDisplay(confusion_matrix=cm_all, display_labels=["Term", "Preterm"])
